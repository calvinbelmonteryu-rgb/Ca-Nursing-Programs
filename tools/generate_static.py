        ...
        <div class="modal-content modal-shortcuts">
            ...
            <div class="shortcuts-grid">
                ...
                <div class="shortcut-group">
                    ...
                    <div class="shortcut-row"><kbd>e</kbd> <span>Quick status picker (selected row)</span></div>
                    <div class="shortcut-row"><kbd>x</kbd> <span>Toggle favorite (selected row)</span></div>
                    <div class="shortcut-row"><kbd>p</kbd> <span>Toggle pin to top (selected row)</span></div>
...
document.addEventListener('DOMContentLoaded', function() {{
    ...
    document.addEventListener('keydown', function(e) {{
        ...
        if (!isEditing(e.target)) {{
            ...
            // 'p' — toggle pin for selected row
            if (e.key === 'p' && selectedRowIdx >= 0 && selectedRowIdx < visibleRows.length) {{
                var pinId = parseInt(visibleRows[selectedRowIdx].dataset.id);
...
function checkBrowserNotifications() {{
    ...
    if (window._browserNotifSent) return;
    var savedStatuses = loadSavedStatuses();
    var today = new Date(); today.setHours(0,0,0,0);
    var urgent = [];
    PROGRAMS.forEach(function(p) {{
        var st = savedStatuses[p.id] || p.application_status || 'Not Started';
        if (st === 'Rejected' || st === 'Offer') return;
        var closeD = parseDate(p.app_close_date);
        var openD = parseDate(p.app_open_date);
        if (closeD && openD && openD <= today && closeD >= today) {{
            var daysLeft = Math.ceil((closeD - today) / 86400000);
            if (daysLeft <= 2) {{
                urgent.push({{ name: p.hospital, days: daysLeft }});
            }}
        }}
    }});
    if (urgent.length === 0) return;
    window._browserNotifSent = true;
    if (Notification.permission === 'granted') {{
        sendBrowserNotif(urgent);
    }} else if (Notification.permission !== 'denied') {{
        Notification.requestPermission().then(function(perm) {{
            if (perm === 'granted') sendBrowserNotif(urgent);
        }});
    }}
}}

function sendBrowserNotif(urgent) {{
    var body = urgent.map(function(u) {{
        return u.name + (u.days === 0 ? ' — LAST DAY!' : ' — ' + u.days + 'd left');
    }}).join('\\n');
    try {{
        new Notification('Urgent: Application Deadlines', {{
            body: body,
            icon: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">\\uD83C\\uDFE5</text></svg>',
            tag: 'rn-tracker-urgent'
        }});
    }} catch(e) {{}}
}}

function updatePageTitle() {{
    var baseTitle = 'CA New Grad RN Tracker';
    var today = new Date(); today.setHours(0,0,0,0);
    var savedStatuses = loadSavedStatuses();
    var urgent = [];
    PROGRAMS.forEach(function(p) {{
        ...
        if (closeD && openD && openD <= today && closeD >= today) {{
            var daysLeft = Math.ceil((closeD - today) / 86400000);
            if (daysLeft <= 3) {{
                urgent.push({{ name: p.hospital.split(' ')[0], days: daysLeft }});
            }}
        }}
    }});
    ...
    if (urgent.length > 0) {{
        var u = urgent[0];
        var warn = u.days === 0 ? 'LAST DAY' : u.days + 'd left';
        document.title = '\u26A0\uFE0F ' + u.name + ' closes ' + warn + ' | ' + baseTitle;
    }} else {{
        // Show active application count
        var activeCount = 0;
        PROGRAMS.forEach(function(p) {{
            var s = savedStatuses[p.id] || p.application_status || 'Not Started';
            if (s === 'Submitted' || s === 'Interview') activeCount++;
        }});
        if (activeCount > 0) {{
            document.title = '(' + activeCount + ') ' + baseTitle;
        }} else {{
...
function renderCmdResults(query) {{
    ...
    var actions = [
        ...
        {{ label: 'Show Favorites Only', action: function() {{ filterFavorites(document.getElementById('fav-chip')); }}, icon: '\\u2605' }},
        {{ label: 'Show My Applications', action: function() {{ filterMyApps(document.getElementById('myapps-chip')); }}, icon: '\\uD83D\\uDCCB' }},
        {{ label: 'Show Open Programs', action: function() {{ filterOpen(); }}, icon: '\\u2714' }},
        {{ label: 'Clear All Filters', action: function() {{ clearAllFilters(); }}, icon: '\\u2715' }},
        {{ label: 'Keyboard Shortcuts Help', action: function() {{ toggleShortcutHelp(); }}, icon: '?' }},
        {{ label: 'Restore Backup', action: function() {{ restoreData(); }}, icon: '\\u2B06' }},
        {{ label: 'Edit Application Goal', action: function() {{ editGoal(); showView('stats'); }}, icon: '\\uD83C\\uDFAF' }},
        {{ label: 'Export Calendar (ICS)', action: function() {{ exportICS(); }}, icon: '\\uD83D\\uDCC5' }},
        {{ label: 'Toggle Compact Density', action: function() {{ toggleDensity(); }}, icon: '\\u2261' }},
        {{ label: 'Share Current URL', action: function() {{ shareUrl(); }}, icon: '\\uD83D\\uDD17' }}
    ];
...
var TAG_COLORS = {{
    'priority': '#ef4444', 'top pick': '#ef4444', 'urgent': '#ef4444',
    'dream': '#8b5cf6', 'reach': '#8b5cf6',
    'safety': '#22c55e', 'backup': '#22c55e',
    'research': '#f59e0b', 'info session': '#f59e0b',
    'local': '#3b82f6', 'nearby': '#3b82f6',
    'competitive': '#ec4899', 'hard': '#ec4899'
}};
...
function renderTableTags() {{
    ...
    document.querySelectorAll('.sheet tbody tr').forEach(function(row) {{
        ...
        // Tag color border
        var tagColor = getTagColor(tags);
        row.style.borderLeft = tagColor ? '3px solid ' + tagColor : '';
        if (tags.length > 0) {{
            var tagHtml = '';
            tags.slice(0, 2).forEach(function(t) {{
                var color = TAG_COLORS[t.toLowerCase()] || '#6b7280';
                tagHtml += '<span class="table-tag" style="border-color:' + color + '">' + t + '</span>';
            }});
...
function renderStats() {{
    ...
    html += '<div class="stats-num-card"><span class="stats-big-num" data-target="' + total + '">' + total + '</span><span>Total Programs</span></div>';
    html += '<div class="stats-num-card"><span class="stats-big-num stat-green" data-target="' + openNow + '">' + openNow + '</span><span>Open Now</span></div>';
    html += '<div class="stats-num-card"><span class="stats-big-num" style="color:#f59e0b" data-target="' + upcoming30 + '">' + upcoming30 + '</span><span>Closing in 30d</span></div>';
    html += '<div class="stats-num-card"><span class="stats-big-num" style="color:#8b5cf6" data-target="' + favs.length + '">' + favs.length + '</span><span>Favorites</span></div>';
    html += '</div>';
    ...
    document.querySelectorAll('.stats-big-num[data-target]').forEach(function(el) {{
        animateCountUp(el, parseInt(el.getAttribute('data-target')), 800);
    }});

    // Animate goal bar fill
    var goalFill = document.querySelector('.goal-fill[data-pct]');
    if (goalFill) {{
        var pct = parseInt(goalFill.getAttribute('data-pct'));
        goalFill.style.width = '0%';
        requestAnimationFrame(function() {{
            goalFill.style.transition = 'width 1s cubic-bezier(0.4, 0, 0.2, 1)';
            goalFill.style.width = pct + '%';
        }});
        // Goal completion celebration
        if (pct >= 100) {{
            var goalCelebrated = localStorage.getItem('rn_tracker_goal_celebrated');
            var goalTarget = localStorage.getItem('rn_tracker_goal') || '10';
            if (goalCelebrated !== goalTarget) {{
                localStorage.setItem('rn_tracker_goal_celebrated', goalTarget);
                setTimeout(function() {{
                    launchConfetti();
                    showToast('Goal reached! You applied to ' + goalTarget + ' programs!');
                }}, 1100);
            }}
        }}
    }}
